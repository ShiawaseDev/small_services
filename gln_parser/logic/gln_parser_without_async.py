...
def validate_construction_date(construction_date):
    # (IV квартал 2022)
    construction_year = str(construction_date).split(' ')[1]
    if int(construction_year) == params['minimal_year_of_building_finished']:
        return True
    return False
...
def main():
    ...
    while True:
        try:
            params['page_number'] = int(input('Enter a number page:'))
        except ValueError:
        ...
        # Exit from prog without warning on typing ctrl+z
            ...
            # get number page for parsing from input
            search_url = str(params['search_url']).format(params['page_number'])
            # return list do not parsed of scanned urls from html
